...
module.exports = (args, config) => {
  if (args.help === true || args.h === true) {
    printHelp(
      "reports detail TASK",
      "Displays detailed information for a specified task",
      {}
    );
    return;
    ...
    getTask(_.join(args._, " "), config, id => {
      query(
        `get(id:\"${id}\"){id,title,parents{id,title},children{id,title},tags,priority,entryDate,dueDate,doneDate,modifiedDate,times,hidden,urg}`,
        config
      )
        .then(data => {
          data = data.get;
          printData = {
            ...
            doneDate: moment(data.doneDate).format("YYYY-MM-DD HH:mm:ss"),
            modifiedDate: moment(data.modifiedDate).format(
              "YYYY-MM-DD HH:mm:ss"
            ),
            times: _.map(
              data.times,
              int =>
                `${moment(int[0]).format("YYYY-MM-DD")}: ${moment(
                  int[0]
                ).format("HH:mm")}-${moment(int[1]).format(
                  "HH:mm"
                )} [${duration(int[0], int[1])}]`
            ),
            totalTime: duration(
              _.sum(_.map(data.times, int => int[1] - int[0]))
            ),
          ...
          for (const k in printData) {
            if (Array.isArray(printData[k])) {
              if (printData[k].length !== 0)
                vLength = Math.max(
                  vLength,
                  _.max(printData[k], o => o.length).length
                );
              else printData[k] = "";
            } else {
          ...
          console.log(
            chalk.bold.underline(
              ("  " + printData.title).padEnd(kLength + vLength + 2)
            )
          );
          var i = 0;
          for (const k in printData) {
            if (Array.isArray(printData[k])) {
              if (i % 2 == 0)
                console.log(
                  chalk.bold(k.padStart(kLength)) +
                    "  " +
                    printData[k][0].padEnd(vLength)
                );
              else
                console.log(
                  chalk.bgBlack(
              ...
              if (printData[k].length > 1) {
                ...
                for (const j in printData[k]) {
                  if (i % 2 == 0)
                    console.log(
                      chalk.bold("".padStart(kLength)) +
                        "  " +
                        printData[k][j].padEnd(vLength)
                    );
                  else
                    console.log(
                      chalk.bgBlack(
